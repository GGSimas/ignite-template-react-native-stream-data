...
function AuthProvider({ children }: AuthProviderData) {
  ...
  async function signIn() {
    try {
      ...
      console.log(authUrl)
      const authLoginResponse = await startAsync({ authUrl });
      // verify if startAsync response.type equals "success" and response.params.error differs from "access_denied"
      // if true, do the following:
      if (authLoginResponse.type === 'success' && authLoginResponse.params.error !== 'access_denied') {
        if (authLoginResponse.params.state !== STATE) {
          throw new Error('Invalid state value');
        }
         // add access_token to request's authorization header
         api.defaults.headers.authorization = `Bearer ${authLoginResponse.params.access_token}`;
         ...
         setUserToken(authLoginResponse.params.access_token);
      }
    } catch (error) {
      throw Error();
    } finally {
  ...
  async function signOut() {
    try {
      ...
      const revokeCall = twitchEndpoints.revocation +
      `?client_id=${CLIENT_ID}` +
      `&`
    } catch (error) {
    } finally {
